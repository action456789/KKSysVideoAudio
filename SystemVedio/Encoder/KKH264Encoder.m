...
#import "KKH264Encoder.h"
#import <VideoToolbox/VideoToolbox.h>
#import "KKVideoConfig.h"

@interface KKH264Encoder()
@property (nonatomic, strong) KKVideoConfig *config;
@end

@implementation KKH264Encoder
{
...
+ (instancetype)encoderWithConfig:(KKVideoConfig *)config {
    KKH264Encoder *encoder = [[KKH264Encoder alloc] init];
    encoder.config = config;
    
    [encoder initCompressionSession];
...
{
    KKH264Encoder *bself = (__bridge KKH264Encoder *)VTref;
...
//        NSLog(@"NALU data lenth: %d, offset: %zu, data lenth: %lu", NALUnitLength, offset, (unsigned long)data.length);
    }
}
