...
@interface FJFaceDetector () <CvVideoCameraDelegate>
{
...
- (void)detectAndDrawFacesOn:(Mat&) img scale:(double) scale
{
    ...
    // 如果对某一帧进行人脸检测的时间超过 1/30 秒，就会产生掉帧现象
...
//        self->eyesDetector.detectMultiScale( smallImgROI, nestedObjects, 1.1, 2, 0, cv::Size(5, 5) );
...
//        }
    ...
    @synchronized(self) {
        ...
        NSArray *array = [self detectedFaces];
        NSMutableArray *images = [NSMutableArray array];
        for (int i = 0; i<array.count; i++) {
            UIImage *img = [self faceWithIndex:i];
            [images addObject:img];
        }
        
        if (images.count > 0) {
            if ([self.delegate respondsToSelector:@selector(faceDetectorResult:)]) {
                [self.delegate faceDetectorResult:images];
            }
        }
    }
}
